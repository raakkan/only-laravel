...
trait HasColorSettings
{
    ...
    public function getColorSettingFields()
    {
        ...
        if ($this->backgroundType == BackgroundType::COLOR) {
            $fields = [
                ColorPicker::make('color.background.color')->label('Background Color')->default($this->background['color']),
            ];
        }

        if ($this->backgroundType == BackgroundType::IMAGE) {
            $fields = [
                TextInput::make('color.background.image')->label('Background Image')->default($this->background['image']),
            ];
        }
