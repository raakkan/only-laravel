...
final class Theme
{
    ...
    {
        return Config::get('only-laravel::themes.symlink_path', 'themes') . '/' . mb_strtolower($this->vendor) . '/' . mb_strtolower($this->name) . ($path ? '/' . $path : '');
    }
    ...
    protected function assertPublicAssetsPath(): void
    {
        ...
        if (file_exists($themeAssetsPath)) {
            ...
            if (! file_exists($publicThemeAssetsPath)) {
                if (Config::get('only-laravel::themes.symlink_relative', false)) {
